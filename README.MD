...
[![Maintainability](https://api.codeclimate.com/v1/badges/0d1d7fbf53440f6372f0/maintainability)](https://codeclimate.com/github/Hakky54/mutual-tls-ssl/maintainability)
[![Codacy Badge](https://api.codacy.com/project/badge/Grade/70a8a5e2a32e4a3c85e33060c326caf0)](https://www.codacy.com/manual/Hakky54/mutual-tls-ssl?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=Hakky54/mutual-tls-ssl&amp;utm_campaign=Badge_Grade)
[![codecov](https://codecov.io/gh/Hakky54/mutual-tls-ssl/branch/master/graph/badge.svg)](https://codecov.io/gh/Hakky54/mutual-tls-ssl)
[![MIT license](https://img.shields.io/badge/License-MIT-blue.svg)](https://github.com/Hakky54/mutual-tls-ssl/blob/master/LICENSE)
...
6. [Automated script for one way authentication with TLS](#automated-script-for-one-way-authentication-with-tls)
7. [Automated script for two way authentication with TLS by trusting each other](#automated-script-for-two-way-authentication-with-tls-by-trusting-each-other)
8. [Automated script for two way authentication with TLS by trusting the Certificate Authority](#automated-script-for-two-way-authentication-with-tls-by-trusting-the-certificate-authority)
...
## Automated script for one way authentication with TLS

You can also automate the creation of the server certificates including trusting the server certificate by the client, updating the application properties and the client configuration with the provided script at the root directory. Run it with the following command: `./configure-one-way-authentication`

## Automated script for two way authentication with TLS by trusting each other

You can also automate the creation of the client and server certificates including trusting the both parties by their specific certificate, updating the application properties and the client configuration with the provided script at the root directory. Run it with the following command: `./configure-two-way-authentication-by-trusting-each-other my-company-name`
The provided argument will be used as CN (Common Name) when creating the client certificate.

## Automated script for two way authentication with TLS by trusting the Certificate Authority

You can also automate the creation of the client and server certificates including the signing process, updating the application properties and the client configuration with the provided script at the root directory. Run it with the following command: `./configure-two-way-authentication-by-trusting-root-ca my-company-name`
The provided argument will be used as CN (Common Name) when creating the client certificate.
